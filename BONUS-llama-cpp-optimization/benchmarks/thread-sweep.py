...
LLAMA_BENCH_EXE_RELEASE = LLAMA_BENCH.parent / "Release" / "llama-bench.exe"

# llama-bench prints a markdown-ish table; grab any tg* (decode) row like tg64/tg128.
TG_RE = re.compile(r"\|\s*tg\d+\s*\|\s*([0-9.]+)\s*±")


def find_bench() -> Path:
    for p in (LLAMA_BENCH, LLAMA_BENCH_EXE, LLAMA_BENCH_EXE_RELEASE):
        if p.exists():
            return p
